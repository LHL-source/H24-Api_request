...
const getDataOnDom = async(doek) => {
        ...
        var loadList_1 = list.map((eachLoadList) => {
            ...
            getDelBtn.addEventListener('click', (eventDelBtn) => {
                console.log('enter eventlistener delBtn');
                console.log('eachLoadList._id', eachLoadList._id); //w? y
                // eachLoadList_Id_2 = eachLoadList._id;
                const deleteTodo = (todoDomElement) => {
                        const id = eachLoadList._id;
                        console.log('id', id); //W?y
...
/* start eventlistener to newDelBtn , lees ui tde eachLoadList._id
...
1: line 23 = to make input empty W? not yet
...
6: trash icon w? not yet almost the test is finished and works good job 1juni
7: delete dom input, button delete, checkbox W? not yet
8: na click op deletebtn: li enz gelijk laten verdwijnen (niet enkel na refresh van page)
*/